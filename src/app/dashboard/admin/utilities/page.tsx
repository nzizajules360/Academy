...
import { collection, doc, updateDoc, arrayUnion, arrayRemove, query, where, getDoc } from 'firebase/firestore';
...
export default function UtilitiesPage() {
  ...
  const [materials, loadingMaterials, errorMaterials] = useCollectionData(materialsQuery, { idField: 'id' });
  ...
  const handleUtilityChange = async (studentId: string, materialId: string, isChecked: boolean) => {
    ...
    try {
        ...
        const utilityToToggle = { materialId, status: isChecked ? 'missing' : 'present' };
        
        await updateDoc(studentRef, {
            utilities: arrayRemove(utilityToToggle)
        });
  ...
  const isLoading = loadingStudents || loadingTerm || loadingMaterials;

  if (isLoading) {
      return <div className="flex justify-center items-center h-64"><Loader2 className="h-8 w-8 animate-spin" /></div>;
  }
  
  if (errorStudents || errorMaterials) {
    return <div className="text-destructive p-4">Error: {errorStudents?.message || errorMaterials?.message}</div>
  }
