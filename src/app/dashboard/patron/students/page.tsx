...
import { Loader2, Pencil, Users, BedDouble, Search, Phone, UserCircle, Shield, Filter } from 'lucide-react';
        ...
        <AccordionItem value={className} className="border-b-0 mb-4 overflow-hidden rounded-xl border bg-card/50 backdrop-blur-sm shadow-lg hover:shadow-xl transition-all duration-300">
          <AccordionTrigger className="p-5 text-lg font-semibold hover:no-underline hover:bg-accent/50 transition-colors group">
            <div className="flex items-center gap-4 w-full">
                <motion.div 
                    className="p-3 bg-gradient-to-br from-blue-500/20 to-cyan-500/10 rounded-xl border border-blue-500/20"
          ...
          <AccordionContent className="bg-gradient-to-br from-accent/20 to-accent/5 p-4 space-y-3">
            {studentsByClass[className].map((student, studentIdx) => (
              <motion.div
                ...
                className="p-4 rounded-lg bg-card/70 backdrop-blur-sm border border-border/50 hover:border-blue-500/30 hover:shadow-md transition-all duration-200"
              >
                <div className="flex flex-col lg:flex-row lg:items-center gap-4">
                  {/* Student Info */}
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    <Avatar className="h-12 w-12 border-2 border-blue-500/20">
                      <AvatarFallback className="bg-gradient-to-br from-blue-500/20 to-cyan-500/10 text-blue-600 font-semibold">
    ...
    <div className="min-h-screen pb-12">
      ...
      <div className="space-y-6">
        ...
        <Card className="bg-card/50 backdrop-blur-sm border-border/50 shadow-xl overflow-hidden">
              ...
              <AnimatePresence mode="wait">
                {filteredStudents.length > 0 ? (
                  ...
                  <motion.div 
                    ...
                    className="text-center text-muted-foreground py-16 border-2 border-dashed rounded-xl bg-gradient-to-br from-blue-500/5 to-cyan-500/5"
                  >
                    <motion.div
                      animate={{ y: [0, -10, 0] }}
                      transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }}
                    >
                      <Users className="mx-auto h-16 w-16 mb-4 text-blue-500/50"/>
                    </motion.div>
                    ...
                    <p className="mt-2 max-w-md mx-auto">
                      {hasActiveFilters 
                        ...
                        : activeTermId 
                          ? "No male students found for the active term." 
          ...
          <CardFooter className="relative z-10 bg-gradient-to-r from-blue-500/10 to-cyan-500/10 border-t border-border/50">
